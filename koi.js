...
const Kirin=require('./Kirin_Koi.js').utils;
...
async function getHelpInfoForCk(cookieIndex, cookie) {
    ...
    data = await with_retry("开启红包活动", async () => {
		$.action='startTask'
		body=await Kirin.get_risk_result($)
	return await requestApi('h5launch', cookie,body);
    })
	//console.log(data)
...
async function openRedPacket(cookie) {
	...
	$.action='h5receiveRedpacketAll'
	const body=await Kirin.get_risk_result($)
    let resp = await requestApi('h5receiveRedpacketAll', cookie,body);
...
async function helpThisUser(help, tool) {
    // 实际发起请求
	$.action='jinli_h5assist'
	$.redPacketId=help.redPacketId
	//console.log("被助力的ID为:"+$.redPacketId)
	const body=await Kirin.get_risk_result($)
	//console.log(body)
